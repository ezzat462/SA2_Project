...
namespace DriveShare.RentalService.Services
{
    public class CarService : ICarService
    {
        ...
        public async Task<ApiResponse<CarPost>> CreateCarAsync(CarPost car)
        {
            ...
            {
                message = $"A new {car.Brand} {car.Model} has been listed for review.",
                type = "NewCarListing",
                carId = car.Id
            });
        ...
        public async Task<ApiResponse<bool>> ApproveCarAsync(int carId)
        {
            ...
            await _kafkaProducer.ProduceAsync("user-targeted-topic", new DriveShare.Shared.Events.UserTargetedEvent
            {
                UserId = car.OwnerId,
                Type = "CarApproved",
                Message = $"Great news! Your car listing '{car.Title ?? car.Model}' has been approved and is now live."
            });
        ...
        public async Task<ApiResponse<bool>> RejectCarAsync(int carId)
        {
            ...
            int ownerId = car.OwnerId;
            string carName = car.Title ?? car.Model;
            ...
            await _kafkaProducer.ProduceAsync("user-targeted-topic", new DriveShare.Shared.Events.UserTargetedEvent
            {
                UserId = ownerId,
                Type = "CarRejected",
                Message = $"Your car listing '{carName}' was rejected by the admin."
            });
