...
namespace DriveShare.UserService.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<ApiResponse<TokenResponseDto>> RegisterAsync(RegisterDto registerDto)
        {
            ...
            Console.WriteLine($"[KAFKA] Publishing event for new registration: {user.FullName}");
            await _kafkaProducer.ProduceAsync("user-targeted-topic", new UserTargetedEvent
            {
                UserId = 0,
